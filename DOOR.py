...
global parametersList_DOOR
parametersList_DOOR = [0]*21


class DOOR():
    
    def __init__(self, name, LRL, URL, FAVD, AA, VA, APW, VPW, MSR, AT, ReaT, ResF, RecT): #parameters
        ...
        self.ResF = ResF
        self.RecT = RecT
        parametersList_DOOR[0] = 1
        parametersList_DOOR[1] = 1
        parametersList_DOOR[2] = LRL   
        parametersList_DOOR[3] = URL
        parametersList_DOOR[4] = AA*10
        parametersList_DOOR[5] = VA*10
        parametersList_DOOR[6] = APW*100
        parametersList_DOOR[7] = VPW*100
        parametersList_DOOR[12] = FAVD
        parametersList_DOOR[13] = MSR
        parametersList_DOOR[14] = AT
        parametersList_DOOR[15] = ReaT
        parametersList_DOOR[16] = ResF
        parametersList_DOOR[17] = RecT
        parametersList_DOOR[18] = 1
        parametersList_DOOR[19] = 16
        parametersList_DOOR[20] = 55
    ...
    def set_from_csv(self):#Obtains values from 1nd row in the users CSV file
        ...
        self.LRL = int(df.loc[5, "P1"])
        self.URL = int(df.loc[5, "P2"])
        self.FAVD = float(df.loc[5, "P3"])
        self.AA = float(df.loc[5, "P4"])
        self.VA = float(df.loc[5, "P5"])
        self.APW = float(df.loc[5, "P6"])
        self.VPW = float(df.loc[5, "P7"])
        self.MSR = float(df.loc[5, "P8"])
        self.AT = str(df.loc[5, "P9"])
        self.ReaT = float(df.loc[5, "P10"])
        self.ResF = float(df.loc[5, "P11"])
        self.RecT = float(df.loc[5, "P12"])

        parametersList_DOOR[0] = 1
        parametersList_DOOR[1] = 1
        parametersList_DOOR[2] = self.LRL   
        parametersList_DOOR[3] = self.URL
        parametersList_DOOR[4] = self.AA*10
        parametersList_DOOR[5] = self.VA*10
        parametersList_DOOR[6] = self.APW*100
        parametersList_DOOR[7] = self.VPW*100
        parametersList_DOOR[12] = self.FAVD
        parametersList_DOOR[13] = self.MSR
        parametersList_DOOR[14] = self.AT
        parametersList_DOOR[15] = self.ReaT
        parametersList_DOOR[16] = self.ResF
        parametersList_DOOR[17] = self.RecT
        parametersList_DOOR[18] = 1
        parametersList_DOOR[19] = 16
        parametersList_DOOR[20] = 55
    ...
    def change(self):
        layout = [
            ...
            [sg.Text('AA: ' + str(self.APW) + "V",key="t4"), sg.Button('+', key="B4+"), sg.Button('-', key="B4-")],
            [sg.Text('VA: ' + str(self.URL) + "V",key="t5"), sg.Button('+', key="B5+"), sg.Button('-', key="B5-")],
            [sg.Text('APW: ' + str(self.AA) + "ms",key="t6"), sg.Button('+', key="B6+"), sg.Button('-', key="B6-")],
            [sg.Text('VPW: ' + str(self.APW) + "ms",key="t7"), sg.Button('+', key="B7+"), sg.Button('-', key="B7-")],
